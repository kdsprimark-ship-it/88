...
const ExportInfo: React.FC = () => {
  ...
  const handleAdd = () => {
    if (!newName) return;
    addBusinessEntity({ type: activeList, name: newName });
    setNewName('');
  };
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className="w-full md:w-64 space-y-2">
          {['SHIPPER', 'BUYER', 'DEPOT'].map((type) => (
            <button
              key={type}
              onClick={() => setActiveList(type as any)}
              className={`w-full text-left px-5 py-4 rounded-2xl font-bold text-sm transition-all ${
                activeList === type ? 'bg-slate-900 text-white shadow-xl translate-x-1' : 'bg-white text-slate-500 hover:bg-slate-50'
              }`}
            >
              Manage {type}s
            </button>
          ...
          <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100">
            <div className="flex items-center justify-between mb-8">
              ...
              <div className="flex gap-2">
                 <button className="p-2 bg-slate-50 text-slate-400 rounded-xl"><Icons.Download size={18} /></button>
                 <button className="p-2 bg-slate-50 text-slate-400 rounded-xl"><Icons.Printer size={18} /></button>
              </div>
            </div>
            ...
            <div className="flex gap-3 mb-8">
              <input 
                type="text" 
                placeholder={`New ${activeList} Name...`} 
                className="flex-1 px-4 py-3 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-slate-200"
                value={newName}
              ...
              <button 
                onClick={handleAdd}
                className="px-6 py-3 bg-indigo-500 text-white rounded-2xl font-bold text-sm hover:bg-indigo-600 transition-all shadow-lg shadow-indigo-100"
              >
                Add {activeList}
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {currentList.map(item => (
                <div key={item.id} className="group flex items-center justify-between p-4 bg-slate-50 rounded-2xl hover:bg-white hover:shadow-md transition-all border border-transparent hover:border-slate-100">
                  ...
                  <button 
                    onClick={() => removeBusinessEntity(item.id)}
