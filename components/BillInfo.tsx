...
const BillInfo: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!matchedEntry) return alert('Valid Invoice required!');
          ...
          {matchedEntry ? (
            <div className="p-8 bg-slate-50 rounded-[32px] grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 animate-in zoom-in-95 duration-300">
              <div className="space-y-1">
                <p className="text-[10px] font-black text-slate-400 uppercase">SHIPPER</p>
                <p className="text-sm font-bold text-slate-900">{matchedEntry.shipperName}</p>
              </div>
              <div className="space-y-1">
                <p className="text-[10px] font-black text-slate-400 uppercase">INDIAN AMOUNT</p>
                <p className="text-sm font-bold text-slate-900">৳{matchedEntry.totalTaka.toLocaleString()}</p>
              </div>
              <div className="space-y-1">
                <p className="text-[10px] font-black text-slate-400 uppercase">DOC AD-ON (165/doc)</p>
                <p className="text-sm font-bold text-indigo-600">৳{(matchedEntry.doc * 165).toLocaleString()}</p>
              </div>
              <div className="space-y-1 bg-white p-4 rounded-2xl shadow-sm border border-slate-100">
                <p className="text-[10px] font-black text-indigo-500 uppercase">TOTAL BILL</p>
                <p className="text-2xl font-black text-indigo-900">৳{totalBill.toLocaleString()}</p>
              </div>
              
              <div className="lg:col-span-4 grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4 border-t border-slate-200">
                <div className={`p-4 rounded-2xl ${dueBill === 0 ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-700'} border border-transparent`}>
                   <p className="text-[10px] font-black uppercase">DUE BILL</p>
                   <p className="text-xl font-black">৳{dueBill.toLocaleString()}</p>
                </div>
                {miscApprove > 0 && (
                  <div className="p-4 rounded-2xl bg-amber-50 text-amber-700 border border-amber-100">
                     <p className="text-[10px] font-black uppercase">MISCNIAS APPROVE BILL</p>
                     <p className="text-xl font-black">+ ৳{miscApprove.toLocaleString()}</p>
                  </div>
                )}
              </div>
            </div>
          ) : formData.invoiceNo && (
            <div className="p-8 bg-rose-50 rounded-3xl text-rose-600 font-bold text-center border border-rose-100 italic">
               Invoice not found in system. Please verify Indian Entry first.
            </div>
          )}

          <button type="submit" disabled={!matchedEntry} className={`w-full py-5 bg-slate-900 text-white font-black rounded-[20px] hover:bg-slate-800 disabled:opacity-20 disabled:grayscale transition-all shadow-xl tracking-widest text-sm`}>
            CONFIRM & SUBMIT BILL
          </button>
        </form>
      </div>
              ...
              {billInfos.map((b, idx) => (
                <tr key={b.id} className={`${idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/20'} hover:bg-slate-100 transition-colors`}>
                  ...
                  <td className="px-8 py-6">
                    <button onClick={() => { if(confirm('Delete Bill?')) deleteBillInfo(b.id); }} className="p-2 text-rose-300 hover:text-rose-500 hover:bg-rose-50 rounded-xl transition-all"><Icons.Trash2 size={16} /></button>
                  </td>
                </tr>
              ))}
            </tbody>
