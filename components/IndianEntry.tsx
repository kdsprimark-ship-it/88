...
const IndianEntryForm: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.invoiceNo) return alert('Invoice required');
    
    if (isEditing) {
      updateIndianEntry(isEditing, { ...formData, totalTaka: calculatedTotal });
      setIsEditing(null);
    } else {
      addIndianEntry({ ...formData, totalTaka: calculatedTotal });
    }
      ...
      <div className="bg-white p-8 rounded-[40px] shadow-sm border border-slate-100">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <div className="space-y-2">
            ...
            <input 
              type="date" required
              className="w-full p-4 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-indigo-500 font-semibold"
              value={formData.date}
          ...
          <div className="space-y-2">
            ...
            <input 
              type="text" required placeholder="e.g. 10223-A"
          ...
          <div className="space-y-2">
            ...
            <select 
              required className="w-full p-4 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-indigo-500 font-semibold"
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-6 col-span-1 md:col-span-2 lg:col-span-3 pt-8 border-t border-slate-50 mt-4">
            {[
              ...
              { id: 'ctn', label: 'CTN' },
              { id: 'ton', label: 'TON' },
              { id: 'truckUnload', label: 'TRUCK UNLOAD' },
              { id: 'con', label: 'CON' },
              ...
              <div key={item.id} className="space-y-2 group">
                ...
                <input 
                  type="number" 
                  className="w-full p-4 bg-slate-50 border border-transparent rounded-2xl focus:bg-white focus:border-indigo-100 transition-all font-mono"
                  value={renderValue((formData as any)[item.id])} 
                  onChange={(e) => handleNumChange(item.id, e.target.value)} 
                />
              </div>
            ))}
          </div>
          ...
          <div className={`col-span-1 md:col-span-2 lg:col-span-3 bg-slate-900 p-8 rounded-[32px] flex flex-col sm:flex-row items-center justify-between gap-6 mt-6 shadow-2xl`}>
            ...
            <div className="flex gap-4">
              {isEditing && (
                <button 
                  type="button" 
                  onClick={() => { setIsEditing(null); setFormData({ ...formData, invoiceNo: '', doc: 0, ctn: 0, ton: 0, truckUnload: 0, con: 0, others: 0 }); }}
                  className="px-8 py-5 bg-slate-700 text-white font-black rounded-[20px] uppercase tracking-widest text-sm"
                >
                  Cancel
                </button>
              )}
              <button 
                type="submit" 
                className={`w-full sm:w-auto px-12 py-5 bg-gradient-to-r ${activeTheme.from} ${activeTheme.to} text-white font-black rounded-[20px] transition-all hover:scale-105 active:scale-95 shadow-xl uppercase tracking-widest text-sm`}
              >
      ...
      <div className="bg-white rounded-[40px] shadow-sm border border-slate-100 overflow-hidden">
        <div className="p-8 border-b border-slate-100 flex items-center justify-between">
          ...
          <div className="flex gap-2">
             <button className="p-3 bg-slate-50 text-slate-400 rounded-2xl hover:bg-slate-100 transition-all"><Icons.Search size={18} /></button>
             <button className="p-3 bg-slate-50 text-slate-400 rounded-2xl hover:bg-slate-100 transition-all"><Icons.Printer size={18} /></button>
          </div>
        </div>
          ...
          <table className="w-full text-left">
            <thead className="bg-slate-50/50 text-slate-400 text-[10px] font-black uppercase tracking-[0.15em]">
              <tr>
                ...
                <th className="px-8 py-6">Stats (D/C/T)</th>
              ...
              {indianEntries.map((e, idx) => (
                <tr key={e.id} className={`${idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/30'} hover:bg-indigo-50/50 transition-colors`}>
                  ...
                  <td className="px-8 py-6 text-xs font-mono font-bold text-slate-400">
                    <span className="text-blue-500">{e.doc || '0'}D</span> • <span className="text-emerald-500">{e.ctn || '0'}C</span> • <span className="text-amber-500">{e.ton || '0'}T</span>
                  </td>
                  <td className="px-8 py-6 text-sm font-black text-slate-900 text-right">
                    <span className="text-xs font-bold text-slate-300 mr-1">৳</span>
                    {e.totalTaka.toLocaleString()}
                  </td>
                  <td className="px-8 py-6">
                    <div className="flex justify-center gap-2">
                       ...
                       <button onClick={() => { if(confirm('Delete?')) deleteIndianEntry(e.id); }} className="p-2 text-rose-500 hover:bg-rose-50 rounded-lg transition-all"><Icons.Trash2 size={16} /></button>
                    </div>
                  </td>
                </tr>
              ))}
              {indianEntries.length === 0 && (
                <tr>
                  <td colSpan={7} className="px-8 py-20 text-center">
                    <p className="text-slate-300 font-bold italic tracking-widest uppercase">No Entries Found</p>
                  </td>
                </tr>
              )}
            </tbody>
